...
class ExpensePdf
{
    public static function generate(int $claimID, string $status): string|false
    {
        ...
        // Load claim header
        $stmt = $mysqli->prepare('SELECT EC.*, U.fullName, D.deptName FROM tblExpenseClaims EC JOIN tblUsers U ON U.userID=EC.userID JOIN tblDepts D ON D.deptID=EC.deptID WHERE EC.claimID=?');
        ...
        // Load items
        $items=[];
        $it = $mysqli->prepare('SELECT itemName, quantity, unitCost, lineTotal FROM tblExpenseClaimItems WHERE claimID=?');
...
<?php
        ...
        switch(strtolower($status)){
            case 'pending': $wm='PENDING'; break;
            case 'not approved': $wm='NOT APPROVED'; break;
            case 'complete': $wm='COMPLETE'; break;
            case 'approved': $wm='APPROVED'; break;
        }
