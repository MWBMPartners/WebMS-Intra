...
class Mailer
{
    ...
    public static function send(array $to, string $subj, string $html, array $files = []): bool
    {
        ...
        if ($fromAddr === '') { throw new RuntimeException('From address missing'); }

        $msg = [
            'message' => [
                'subject' => $subj,
                'body' => [ 'contentType' => 'HTML', 'content' => $html ],
                'toRecipients' => array_map(fn($e)=>['emailAddress'=>['address'=>$e]], $to),
        ...
        if($code>=200&&$code<300){curl_close($ch);return true;}
        Logger::errorPlatform('Graph','Error',(string)$code,'Mail send fail',$resp);
        curl_close($ch);
        return false;
    }
    ...
    private static function attach(array $paths): array
    {
        $out=[];
        foreach($paths as $p){
            if(!is_file($p)||filesize($p)>4*1024*1024){continue;}
            $out[]=[
    ...
    {
        if(self::$token!=='' && time()<self::$tokenExp-60){return self::$token;}
        global $SETTINGS;
        $cid=$SETTINGS['auth']['ms365']['appwide']['clientID']??'';
        $sec=$SETTINGS['auth']['ms365']['appwide']['clientSecret']??'';
        $tenant=$SETTINGS['auth']['ms365']['tenantID']??'';
        if($cid===''||$sec===''||$tenant===''){throw new RuntimeException('Graph creds missing');}

        $url='https://login.microsoftonline.com/'.$tenant.'/oauth2/v2.0/token';
        $post=['client_id'=>$cid,'client_secret'=>$sec,'grant_type'=>'client_credentials','scope'=>'https://graph.microsoft.com/.default'];
        $ch=curl_init($url);
        curl_setopt_array($ch,[CURLOPT_POST=>true,CURLOPT_POSTFIELDS=>http_build_query($post),CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>15]);
        $resp=curl_exec($ch);
        if($resp===false){curl_close($ch);throw new RuntimeException('Token request failed');}
        $data=json_decode($resp,true);curl_close($ch);
        if(!isset($data['access_token'])){throw new RuntimeException('Token parse fail');}
        self::$token=$data['access_token'];
        self::$tokenExp=time()+$data['expires_in'];
        return self::$token;
    }
}
