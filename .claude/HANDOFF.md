...
| 5a | **#518 (CRITICAL)** one organisation's administrator can change any account | **RUN STARTED about 06:33: run `wf_dddc82f4-f0a`, task `w9pgkuk6s`**. **Step 1 (reproduce and sweep) DONE on OPUS at 06:52** (Fable out of credits; `p518--sweep.md`). FULL TAKEOVER REPRODUCED on a real DB, and #518 has been commented on. Same fault also in: users/import.php, invites save/accept, offboarding do/rehire/user/index, safeguarding dbs-save/dbs, and the users API session branch (partial); erasure is partial. **Step 2 (draft plan) DONE on OPUS at 07:10** (`p518--plan-draft.md`). **RUN FAILED about 10:03: the Sonnet BUILD agent stalled 6 times** (the first attempt worked from 07:35 to about 08:40 BST, reaching proof section 4, then got no responses; the next 5 attempts stalled with little or no progress, which points to the service rather than a hung command). At 11:40 a Sonnet probe answered READY and Fable was still out of credits. **The build's edits are IN THE WORKING TREE, uncommitted: 16 modified plus 3 new files (774+/102-); status unknown against the plan.** The scratch harness is kept at `/private/tmp/p518-work` (reset.sh, seed.sql, router.php, proof.php sections 2-4) with the container `p518-mysql` (root/rootpass, db webms); the orphan php server on 9061 was stopped. **CONTINUATION RUN wf_af9af0c1-2c9 ALSO FAILED (15:41-18:58):** six Sonnet attempts, then six Opus attempts, were each killed by the WORKFLOW TOOL's 3-minute no-progress rule while the service was extremely slow (single tool results took 10-16 min). The working tree is unchanged, and the Opus attempt confirmed it matches the saved copy. **About 19:10: the builder was restarted as a plain BACKGROUND AGENT (Agent tool, dev-team:sonnet-builder), which has no 3-minute kill**, with its full brief in `.claude-work/briefs/518-continue-build.md`; it appends progress to `.claude-work/resume/p518--build.md`. **LESSON: while the service is slow, run long build and check steps as background agents, not workflow agents.** **BUILD COMPLETE about 19:05** (`p518--build.md`): all 19 plan files were ALREADY correct from the stalled build (each read line by line against the plan); the builder supplied the missing proof: 199 checks on MySQL 8.0.36 + PHP 8.5.10 with two organisations, passing twice from clean runs, plus a before-the-fix run on the same database where all four attacks SUCCEEDED (so the pass is meaningful). Four faults it hit were in its own test script, each proven against the database. I verified independently at 19:12: exactly the 19 files, nothing staged, no stash, php -l clean, every audit check and self-test exit 0, the new check script 0 findings, api-spec valid JSON. **Independent check r1 launched about 19:15 as a background agent on OPUS (Fable still out of credits, probed twice), brief `.claude-work/briefs/518-check-r1.md`, report `.claude-work/resume/p518--verify-r1.md`.** After it reports: a fresh independent checker (also a background agent; Fable first, else Opus), then commit. (Earlier: continuation run started 15:41 as run `wf_af9af0c1-2c9`, task `wlp3homxg`.) (a copy of the partial build is saved at `.claude-work/resume/518-partial-build-1340/`). **Script `.claude-work/resume/wf-518-continue.js`:** a continuation builder establishes each step's status against the plan, finishes, and runs every proof; then the independent check and fix rounds; every agent call falls back to Opus if it stalls. **Step 3 (settle) DONE on OPUS about 07:34** (`p518--plan.md`). The builder is SONNET, and 19 files may change (new AccountGuard.php, account-guard-selftest.php and check_account_writes_guarded.py; users save/index/import; users API update/create; ApiAuth comment; api-spec; offboarding user/do/rehire/index; invites save/accept/new; dbs-save/dbs). **The build has been running since 07:35 using the plan's RECOMMENDED answers to its owner questions.** **Owner answered about 07:45:** Q4, the live portal DOES have several organisations switched on (so the upgrade notes must cover accounts with no membership); Q9, PREPARE a read-only post-deploy checklist for a global administrator (**DRAFTED and TESTED about 07:55**: `.claude-work/pending-518/post-deploy-check-518.md`, 8 read-only checks and 9 queries, all run on MySQL 8.0.36 against a made-up two-organisation example with the expected rows; parked outside the tree so the checker's changed-files test is not confused. **Move it to `docs/post-deploy-check-518.md` and commit it WITH the fix.**); Q5, an 'admin' invitation makes the person administrator of the sending organisation only, including pending invitations; Q2, EVERYTHING on the account of a person in several organisations needs a global administrator. All of these match the plan's recommendations the build is using. The plan suggests follow-up issues: the "email already exists" message reveals accounts; the /admin dashboard shows installation-wide totals; path-mode redirects drop the prefix (compare #512); `Workflow.php:1057` compares a flag with '1'. (script `.claude-work/resume/wf-518.js`). Steps: reproduce and sweep, draft, challenge and settle, build, then checks with up to 3 fix rounds. It commits nothing; outputs go to `.claude-work/resume/p518--*.md`. Resume with the same scriptPath and `resumeFromRunId`. After it passes: run the checks, commit, push, comment on #518, update the handoff. |
| 5a2 | **#519** anonymous check-in page shows internal events, **plus #520** waiting-list promotion always fails silently (owner, about 06:45: done together) | queued straight after #518 |
