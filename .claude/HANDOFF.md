...
| 5a2 | **#519** anonymous check-in page shows internal events, **plus #520** waiting-list promotion always fails silently | **RUN STARTED 20:25, 17 Sept: run `wf_6159bae4-a22`, task `wqws3rcxy`** (script `.claude-work/resume/wf-519-520.js`; plan, challenge and settle, Sonnet build, independent check with fix rounds; every step falls back to Opus if it stalls; nothing committed by the run). Outputs `.claude-work/resume/p519--*.md`. **Draft plan DONE on OPUS 21:23** (Fable out of credits): for #519 a public event stays open to anyone, while an internal event is limited to a member of THAT event's organisation, tested in SQL (the shape #514's P2 adopts, so nothing is unpicked); drafts and deleted events stay refused for everyone. It also proposes teaching `check_sql_columns.py` to read a short table name, and a rate limit on check-ins. **Its prototype found two more never-working pages, both verified by me against the schema at 21:30 and opened as issues: #523 (live chat moderation always 500s, `flaggedReason` should be `flagReason`) and #524 (the leadership list endpoint has never worked, `assignedAt` does not exist).** Both are to be fixed in the same commit, because the improved check reports them. **Owner note (no answer needed): anonymous check-in counts are recorded but nothing reads them** — the plan fixes the page and leaves the counts unread. **Settled plan DONE on OPUS 21:53** (`p519--plan.md`; builder chosen: SONNET). Both faults were re-reproduced by the challenger itself. Files that may change: anon-checkin.php, anon-checkin-save.php, a new migration 197 plus full_schema.sql (the rate limit), Events.php, admin/live/chat.php (#523), leadership/api/list.php (#524), check_sql_columns.py (short table names), CHANGELOG.md, DEV_NOTES.md. **Owner questions (neither blocking): (1) keep the check-in feature? ANSWERED by the owner asking for #525, so KEEP. (2) the check-in limit: **the owner answered about 22:00 — keep ONE setting in this build but change the default from 300 to 500 in five minutes**, and build the four-level version (event > venue > organisation > installation, most specific winning, installation value is a default and NOT a ceiling, 0 switches off) as **#526**, straight after this. **TO DO BEFORE COMMITTING #519/#520: change the seeded default from 300 to 500 in migration 197 and in full_schema.sql, then re-run the migration-idempotency and schema-seed-parity checks.** Build running since 21:53. |
