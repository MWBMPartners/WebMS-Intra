 ...
 * installation uses. That includes the switch for error reporting and the
 * address error reports are sent to.
 *
 ...
 * organisation could send every organisation's error reports, which can contain
 * personal details, to an error-tracking account of their choosing. The owner
...
//    (web/_apps/admin/integrations/monitoring/index.php) was not changed, so
...
if (App::isRootAdmin() === false) {
    Logger::activity(
        ...
        'Refused: portal-wide settings group "monitoring" may only be changed by a global administrator',
