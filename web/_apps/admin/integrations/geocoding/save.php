 ...
 * installation uses. That includes the Google key used to turn addresses into
 * map positions, and the switch that makes the portal look addresses up
 * automatically, which sends them to an outside service.
 *
 ...
 * organisation could switch automatic address look-ups on for everybody, or
 * swap the Google key. The owner decided that settings affecting every
...
//    (web/_apps/admin/integrations/geocoding/index.php) was not changed, so an
...
if (App::isRootAdmin() === false) {
    Logger::activity(
        ...
        'Refused: portal-wide settings group "geocoding" may only be changed by a global administrator',
